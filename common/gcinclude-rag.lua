...
-- Custom chat aliases that should run a command directly (not forwarded to LAC).
-- Add more entries here over time.
local DirectAliases = T{
}
...
function gcinclude.Load()
    ...
    gcinclude.SetDirectAlias(DirectAliases)
...
function gcinclude.Unload()
    ...
    gcinclude.ClearDirectAlias(DirectAliases)
    gcdisplay.Unload()
end
...
function gcinclude.SetDirectAlias(aliasEntries)
    local seen = {}

    for _, v in ipairs(Overrides) do
        seen[tostring(v):lower()] = true
    end
...
function gcinclude.ClearDirectAlias(aliasEntries)
    for _, entry in ipairs(aliasEntries) do
...
function gcinclude.DoDefaultOverride(isMelee)
    ...
    if (player.Status == 'Resting') then
        if (not restTimestampRecorded) then
            restTimestamp = os.clock() + restingEquipDelaySeconds
